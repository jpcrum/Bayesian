...
# initsList = list (list( theta=0.1 ), list( theta=0.9 ), list( theta=0.5 ))
# Option: Use function that generates random values for each chain:
initsList = function() {
  resampledY = sample( y , replace=TRUE ) # bootstrap resampling
...
plotPost( codaSamples[,"theta"] , main="theta" , xlab=bquote(theta) , 
          cenTend="median" , compVal=0.5 , ROPE=c(0.35,0.55) , credMass=0.90 )
